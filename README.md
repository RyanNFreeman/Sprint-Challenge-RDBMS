...
RDBMS is just a structure. It is a program that looks like an excel spreadsheet and has rows/records and columns/fields. SQL is a RDBMS and the command language to create and query the DB.

2. Why do tables need a primary key?

It can be both very bad for a decision or even dangerous for an individual if we don’t have primary keys for their data. We run a higher risk of updating or deleting the wrong data and causing serious issues. Plus, it just doesn’t make sense to not be able to tell the difference between things like duplicate names without referencing a primary key.
...
 We need to introduce a third table with multiple foreign keys.
